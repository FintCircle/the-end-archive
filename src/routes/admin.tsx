...
function Admin() {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-12">
    ...
    <nav className="mt-10 flex flex-wrap gap-2">{[["archive","Archive entries"],["stories","Stories"],["corrections","Corrections"],["reports","Reports"]].map(([value,label]) => <button key={value} onClick={() => setTab(value)} className={`border px-4 py-2 font-mono text-xs uppercase ${tab === value ? "bg-foreground text-background" : ""}`}>{label}</button>)}</nav>
