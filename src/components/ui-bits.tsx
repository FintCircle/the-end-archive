...
export function Header() {
  ...
  const go = (e: FormEvent) => { e.preventDefault(); nav({ to: "/search", search: { q } }); };
  return (
    <header className="border-b border-foreground">
      <div className="mx-auto flex max-w-6xl items-center gap-6 px-4 py-3">
        <Link to="/" className="font-display text-xl font-bold tracking-[0.2em]">SCRUTTIN</Link>
        ...
        <Link to="/add" className="font-mono text-xs uppercase tracking-wider">+ Add</Link>
      </div>
    </header>
  );
}
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-block border px-2 py-0.5 font-mono text-[11px] font-semibold uppercase tracking-widest ${soon ? "border-accent bg-accent text-accent-foreground" : "border-primary text-primary"}`}>
      {status}
    </span>
  );
}
...
export function Lifeline({ start, end, large }: { start: string; end: string; large?: boolean }) {
  return (
    <div className={`flex items-center gap-3 font-mono ${large ? "text-2xl md:text-3xl" : "text-sm"}`}>
...
export function ThingCard({ t }: { t: Thing }) {
  return (
    <Link to="/thing/$slug" params={{ slug: t.slug }} className="group flex flex-col gap-3 border-t border-foreground pt-4">
      <StatusBadge status={t.status} />
      <h3 className="font-display text-2xl font-semibold leading-tight group-hover:underline">{t.name}</h3>
...
export function ResultRow({ t }: { t: Thing }) {
  return (
    <Link to="/thing/$slug" params={{ slug: t.slug }} className="grid gap-1 border-b border-border py-5 md:grid-cols-[1fr_auto] md:items-center">
      <div>
        <h3 className="font-display text-2xl font-semibold hover:underline">{t.name}</h3>
        <p className="text-sm text-muted-foreground">{t.description}</p>
        <p className="mt-1 font-mono text-xs text-muted-foreground">{t.type} · {t.status}</p>
      </div>
      <span className="font-mono text-sm">{year(t.launch)} — {year(t.end)}</span>
    </Link>
  );
}
